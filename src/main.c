...
/*
int8_t getxmap(int16_t x, int16_t y) {
  if (x<0 || x>=MAPSIZE || y<0 || y>=MAPSIZE) return -1;
  return map[(int)y * MAPSIZE + (int)x];
}

void setxmap(int16_t x, int16_t y, int8_t value) {
...
//player pos
  ...
  transx = (player.x*ZOOM)>>6;
  transy = (player.y*ZOOM)>>6;
  
  for (int16_t x = 0; x < mapsize; x++)
    for (int16_t y = 0; y < mapsize; y++) {
      graphics_context_set_fill_color(ctx,  map[y*mapsize+x]>0?1:0);
      dot.origin.x = overlay.origin.x + (x*ZOOM);// + ((ZOOM*mapsize)/2) - transx;
      dot.origin.y = overlay.origin.y + (y*ZOOM);// + ((ZOOM*mapsize)/2) - transy;
      dot.size.w = ZOOM;
      dot.size.h = ZOOM;
    ...
    graphics_draw_pixel(ctx, GPoint(transx+overlay.origin.x, transy+overlay.origin.y));
  ...
  snprintf(text, sizeof(text), " x:%ld y:%ld", (player.x), (player.y));  // What text to draw
  graphics_context_set_fill_color(ctx, 0); graphics_fill_rect(ctx, textframe, 0, GCornerNone);  //Black Filled Rectangle
  graphics_context_set_stroke_color(ctx, 1); graphics_draw_rect(ctx, textframe);                //White Rectangle Border
  graphics_context_set_text_color(ctx, 1);  // Text Color
  graphics_draw_text(ctx, text, fonts_get_system_font(FONT_KEY_GOTHIC_14), textframe, GTextOverflowModeWordWrap, GTextAlignmentLeft, NULL);  //Write Text
...
static void timer_callback(void *data) {
  ...
  int32_t dx = (accel.x>>5);
  int32_t dy = (accel.y>>5);
  if(getmap(player.x + dx, player.y) <= 0 || IDCLIP) player.x += dx;
  if(getmap(player.x, player.y - dy) <= 0 || IDCLIP) player.y -= dy;
